...
(define (encrypt bdy)
  (let* ((p (length bdy))
         (s (if (null? bdy) 0 (apply + (map length bdy))))
         (e (if (null? bdy) 'SCRT (length (car bdy)))))
    (append bdy (list (list 'Secret-code: (list (* p s) (if (eq? e 'SCRT) 'SCRT (* e 100))))))))
    ...
    [(= id (car (car mb-lst)))
     (cons
      (let ((email (car mb-lst)))
        (replace-value 6
          (cond
            [(eq? tag 'conf) (encrypt (list-ref email 6))]
            [(eq? tag 'prsnl) (add-stats (list-ref email 6))]
            [else (list-ref email 6)])
